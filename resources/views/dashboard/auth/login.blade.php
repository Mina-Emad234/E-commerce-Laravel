            ...
            <div class="card border-grey border-lighten-3 m-0">
                <div class="card-header border-0">
                    ...
                    <h6 class="card-subtitle line-on-side text-muted text-center font-small-3 pt-2">
                        <span>{{__('admin/auth.login_header')}} </span>
                    </h6>
                </div>
                ...
                <div class="card-content">
                        ...
                        <form class="form-horizontal form-simple" action="{{route('admin.post.login')}}" method="post"
                            ...
                            <fieldset class="form-group position-relative has-icon-left mb-0">
                                <input type="text" name="email" class="form-control form-control-lg input-lg"
                                       value="{{old('email')}}" id="email" placeholder=" {{__('admin/auth.email_placeholder')}}">
                                ...
                                <div class="col-md-6 col-12 text-center text-md-left">
                                    <fieldset>
                                        <input type="checkbox" name="remember_me" id="remember-me"
                                               class="chk-remember">
                                        <label for="remember-me">{{__('admin/auth.remember_me')}}</label>
                                    </fieldset>
                                </div>
                            </div>
                    ...
                    <ul style="text-align: center;list-style-type: none; padding: 0">
                        @foreach($langs=LaravelLocalization::getSupportedLocales() as $localeCode => $properties)
                            <li style="display: inline;">
                                <a rel="alternate" hreflang="{{ $localeCode }}" href="{{ LaravelLocalization::getLocalizedURL($localeCode, null, [], true) }}">
                                    {{ $properties['native'] }}
                                </a>
                            </li>
                            <div  style="display: inline;" class="{{$properties==end($langs)?'d-none':''}}"> | </div>
                        @endforeach
                    </ul>
                </div>
            </div>
