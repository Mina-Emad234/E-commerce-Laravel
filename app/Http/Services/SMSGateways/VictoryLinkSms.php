...
class VictoryLinkSms
{
    ...
    /**
     * send sms using victory line gate
     * @param $phone
     * @param $message
    ...
    {
        $params = [
            'UserName' => config('sms.VICTORY_LINK.USERNAME'),//credentials
            'Password' => config('sms.VICTORY_LINK.PASSWORD'),
            'SMSText' => $message,
            'SMSLang' => $language == 'ar' ? 'A' : 'E',
            'SMSSender' => config('sms.VICTORY_LINK.SMS_SENDER'),//name of sender of sms
            'SMSReceiver' => $phone,
        ];
